...
runners.filter(function(element){
    if(element.shirt_size === 'L'){
        largeShirts.push(element.first_name + " " + element.last_name + " " + element.email + " " + element.company_name + " " + element.donation)
    }
})
...
// Problem 1  Get emails for each person to email them runners numbers!
let emails = [];
runners.forEach(function (element) {
    emails.push(element.first_name + " " + element.last_name + " " + element.email)
})
console.log(emails);

// Problem 2 swap first to last name for some purpose
